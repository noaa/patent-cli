...
- **Version variable**: `var version = "0.1.5"` in `cmd/gp-cli/main.go`. The release workflow injects it at build time via `-ldflags "-X main.version=${{ github.ref_name }}"`. Must be `var` (not `const`) for ldflags to work.
