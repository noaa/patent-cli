...
| Package | Role |
|---------|------|
| `internal/fetcher` | HTTP client (`FetchHTML`, `FetchBinary`). Normalizes patent numbers for the Google Patents URL format (US 6-digit sequences are zero-padded to 7 digits). Supports proxy and custom CA via `Options`. |
| `internal/parser` | Parses raw HTML → `PatentData`. Uses `goquery` for DOM traversal and regex for `<meta>` / canonical URL extraction. `ParseImageURLs` deduplicates thumbnail vs. high-res by taking the last occurrence of each filename. |
| `internal/formatter` | Converts `PatentData` → `DataMap` (ordered map) → rendered output (`json`/`text`/`tsv`). `text` and `tsv` files are written with a UTF-8 BOM for Excel/Notepad compatibility. `DataMap` has a custom `MarshalJSON` to preserve insertion order. |
...
- **Error output**: structured `{"ok": false, "error": {"type": "…", "message": "…"}}` JSON goes to stdout. Exit codes: 0=ok, 1=general, 4=not-found, 6=server-error. Progress messages go to stderr and are suppressed by `--quiet`.
